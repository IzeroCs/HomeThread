...
- Dashboard hiện đã gửi reply ACK ngay khi nhận ACK IP_ADDR (16 byte) trong `CommandManager.handle()`. Tuy nhiên lúc khởi động / reconnect có thể vẫn thấy 1–2 lần log `ipaddr response no ACK` do timing (ACK reply tới trễ hoặc socket vừa đóng/mở).
...
- **Vị trí:** Tài liệu tập trung tại `namorix-thread/documents/`. Từ `firmware/br-host`: symlink `docs/` → `../../documents/`. Danh mục đầy đủ: **documents/README.md** (ba thành phần: endpoint, br-host, dashboard; bảng danh mục file).
...
2. **Dashboard:** Sửa reply ACK cho CMD_IP_ADDR — trong `CommandManager.handle()` khi nhận ACK (frameId ∈ ipAddrFrameIds, data.length === 16) gọi `replyAck(frame.frameId)` để BR không retry vô hạn.
...
4. **Docs endpoint / dashboard:** Cập nhật hoặc tạo doc (child gửi thẳng backend qua IP, backend listen IP)
...
- **Dashboard:** IP_ADDR response no ACK retry — backend chỉ gửi reply ACK khi `stateChangedOrFirst` trong pullState; các lần fetch IP_ADDR khác không reply → BR retry. Fix: gọi replyAck trong `CommandManager.handle()` cho mọi ACK CMD_IP_ADDR (16 byte).
