...
- [x] `ot_change_detector`: hook `otSetStateChangedCallback()` → debounce (arm-once) → build snapshot (role/rloc/dataset + tables) → diff → `changed_mask` + push `CMD_NOTIFY (0x45)` (payload u32 BE)
...
- [x] Tài liệu hệ thống tại **namorix-thread/documents/**; danh mục: **documents/README.md**. Symlink `docs/` từ `firmware/br-host` root.
...
- [ ] Cập nhật/tạo doc endpoint và dashboard (child gửi thẳng backend, backend listen IP)
...
| BR log `ipaddr response no ACK` 1–2 lần khi boot/reconnect | dashboard | Dashboard đã reply ACK cho IP_ADDR trong `CommandManager.handle()`; 1–2 lần retry thường do timing/reconnect. Nếu spam liên tục thì kiểm tra TCP ổn định và log `Failed to send reply ACK`. |
