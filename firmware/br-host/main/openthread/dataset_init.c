/*
 * Dataset init: khi boot kiểm tra active dataset; nếu không có thì tạo mới (random), đặt tên mạng ESP-BR-<MAC> rồi set active.
 */
...
void openthread_dataset_init_on_boot(void)
{
    ...
    /* Set network name to ESP-BR-<MAC last 2 bytes> */
    uint8_t mac[6];
