...
#ifndef OT_TABLE_SNAPSHOT_H
#define OT_TABLE_SNAPSHOT_H
...
/**
 * Build Router Table snapshot.
 *
 * Format (same as communicate protocol):
 *   - count(1)
 *   - repeated entries (15 bytes each):
 *       RouterId(1) + RLOC16(2) + ExtAddress(8) + LinkQualityIn(1) +
 *       LinkQualityOut(1) + Age(2)
 */
bool ot_snapshot_build_router_table(otInstance *instance, uint8_t *buf, size_t buf_size, size_t *out_len);

/**
 * Build Child Table snapshot.
 *
 * Format (same as communicate protocol):
 *   - count(1)
 ...
 */
bool ot_snapshot_build_child_table(otInstance *instance, uint8_t *buf, size_t buf_size, size_t *out_len);

/**
 * Build Commissioner Joiner Table snapshot.
 *
 * Format (same as communicate protocol):
 *   - count(1)
 ...
 */
bool ot_snapshot_build_joiner_table(otInstance *instance, uint8_t *buf, size_t buf_size, size_t *out_len);

#ifdef __cplusplus
}
#endif

#endif /* OT_TABLE_SNAPSHOT_H */
