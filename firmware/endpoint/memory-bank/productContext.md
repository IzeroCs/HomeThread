...
Endpoint đóng gói toàn bộ phức tạp này thành một API duy nhất.
...
Trong Thread mesh, nếu không có Border Router, một node thường tự trở thành Leader. Khi Border Router kết nối lại, nó cần lấy lại vai trò Leader. Endpoint sử dụng `prefer_not_leader` và `router_selection_jitter` để tránh trở thành Leader khi không mong muốn.
...
```
endpoint                       Backend (địa chỉ từ backend discovery)     Border Router (br-host)
    │                                         │                                        │
...
2. **Hệ thống Namorix Thread** — br-host (Border Router) tương tác với endpoint qua CoAP để thu thập device info và gửi control commands
