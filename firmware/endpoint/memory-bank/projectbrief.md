...
## Vị trí trong hệ thống Namorix Thread

```
[dashboard]  ←── TCP frame protocol ──→  [br-host (ESP32-S3 BR)]
                                                        ↕  CoAP over Thread mesh
                                            [endpoint devices (ESP32-C6/H2)]
```

Endpoint là **lớp thiết bị cuối** (End Device / Router Node) trong hệ thống:
- Kết nối vào Thread mesh thông qua Border Router (br-host)
...
- Border Router setup (thuộc br-host)
...
- `documents/README.md` — Danh mục tài liệu Namorix Thread (architecture, protocol, coap, entity-model, installation, websocket).
