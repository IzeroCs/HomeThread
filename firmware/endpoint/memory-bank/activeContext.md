...
## Focus hiện tại (2026-03-11)

Dự án đang ở giai đoạn **hoàn thiện hạ tầng Backend communication**. Device register **align backend**: (1) POST /device/register/info — device info (keys 0–6, **key 0 = mac_address bstr(8)**); (2) POST /device/register/entity — **key 0** (mac bstr(8)) + **key 1** (array entities). Mỗi entity item gửi ENTITY_KEYS 0–6 (entity_id, name, type, device_class, unit, **restore_mode**, disabled). **device** component ở **components/device/**; **thread** core ở **components/thread/core/** (thread_node, thread_joiner, thread_coap, thread_discovery). Ping và discovery như trước; **chờ register thành công** (retry 2s) rồi mới gửi entities. Công việc còn lại: **Entity CoAP Server** (handlers 5.01) và **CBOR** cho switch/fan/climate/binary_sensor.
...
- **mac_address bstr(8):** Node encode **key 0** (mac) là CBOR **byte string 8 bytes** (EUI-64 IEEE 802.15.4, từ `esp_read_mac(..., ESP_MAC_IEEE802154)`). Không đổi khi factory reset. Helper `mac_uint64_to_bstr()` trong entity_serialization.c; dùng trong info, topology, register/entity, update/state. Spec: `documents/coap/device_payload_spec.md`.
- **Entity restore_mode (key 5 trong ENTITY_KEYS):** Trong register/entity payload, mỗi entity map gửi key 5 = restore_mode (uint). Backend dùng cho mergeEntity. Node: `cbor_register_keys.h` có `CBOR_K_ENT_RESTORE_MODE`; serialize_light/sensor encode với giá trị mặc định 0. Sau có thể thêm vào entity_base_t.
- **Topology role-based + key 6 (neighbors):** Child gửi keys 0–5 (mac bstr(8), rloc16, role=0, parent_rloc16, parent_rssi, parent_lq); Router/Leader gửi keys 0,1,2,6 (mac bstr(8), rloc16, role, array TopologyNeighbor). Hai API: `entity_serialize_topology_child_cbor`, `entity_serialize_topology_router_leader_cbor`. device_registry branch theo role; router/leader thu thập neighbors qua `otThreadGetNextNeighborInfo`. Docs: `documents/coap/device_payload_spec.md` (§2.2).
...
| `components/entity/serialization/entity_serialization.c` | ⚠️ Partial | entity_serialize_info_cbor, entity_serialize_entities_cbor, topology **role-based**, entity_serialize_state_cbor; **mac_address** encode **bstr(8)** (key 0) mọi payload; light+sensor encode ENTITY_KEYS 0–6 (restore_mode key 5); switch/fan/climate/binary_sensor chưa |
| `examples/light_on_off/main/main.c` | ✅ | thread_node_start; on_joined chỉ setup device + entities + entity_coap_server_start |

## Docs

- **Tài liệu hệ thống (repo root):** `documents/README.md` — danh mục toàn bộ tài liệu Namorix Thread (architecture, protocol, coap, entity-model, installation, websocket).
- **Spec chính CoAP (canonical):** `documents/coap/device_payload_spec.md` — endpoints, CBOR keys (device_info 0–6, key 0 = mac bstr(8)), topology role-based, entity/state, DB 8 bảng, flow đăng ký. MAC = IEEE 802.15.4 EUI-64, không đổi khi factory reset.
