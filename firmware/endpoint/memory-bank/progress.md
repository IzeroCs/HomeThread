...
| Danh mục tài liệu | ✅ `documents/README.md` — kiến trúc Namorix Thread, danh sách file (architecture, protocol, coap, entity-model, installation, websocket). |
...
Thread-Node gửi `/device/register` **trực tiếp tới Backend** (không qua BR). Backend (dashboard) nhận CoAP và xử lý; không còn CMD_DATA từ BR cho device register.
