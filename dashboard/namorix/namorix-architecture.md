...
### 8.1 CSS Layer Architecture

| Layer | File | Owner | Mô tả |
|:---|:---|:---|:---|
| 0 — Design Tokens | `tokens.css` | Core | Định nghĩa toàn bộ `--nmx-*` variables. Inject vào `<head>`. Không ai được override. |
| 1 — Base Stylesheet | `nmx-base.css` | Core | Reset, typography, utility classes. Load một lần, toàn hệ thống. |
| 2 — Plugin Stylesheet | `nmx-{pid}.css` | Plugin | Chỉ chứa class `nmx-{pid}-*`. Chỉ được dùng `var(--nmx-*)`. |

> **Load order:** `tokens.css` và `nmx-base.css` luôn có mặt trong `<head>` trước khi Plugin Loader inject bất kỳ Plugin stylesheet nào.

### 8.2 Quy tắc Plugin được và không được tự quyết
...
### 8.4 Utility Classes từ nmx-base.css
...
*End of Document — Namorix Architecture Specification v1.1*
