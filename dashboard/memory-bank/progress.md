# Progress — Namorix Thread (dashboard)
...
| 2.21.0  | **Memory bank — đồng bộ embed shell + core 0.9.2.** Ghi rõ `defineCustomElementOnce` trong `@namorix/core` (tránh đăng ký trùng tag khi Desktop đã load shell rồi dynamic-import `thread.js`). Ghi chú dev: Express plugin backend cần `DESKTOP_ORIGIN` khớp origin Vite Desktop (kể cả port); bundle lib plugin (`vite.plugin.config.ts`) nên `define` `process.env.NODE_ENV` nếu dependency còn tham chiếu `process`. Tham chiếu chéo `namorix/memory-bank/`, `namorix-core/memory-bank/` — **không đổi code** bắt buộc trong mốc này nếu đã align env/build. |
| 2.21.1  | **Docs — HTTP gateway vs WS trực tiếp.** `documents/namorix-core-usage.md` + `dashboard/memory-bank/techContext.md`: REST qua Desktop `/api/plugins/:id/*`; Socket.IO từ plugin UI **trực tiếp** tới Thread backend (không WS gateway Desktop). Khớp `namorix-desktop-architecture.md` §6.11. |
...
- documents/dashboard/migration_to_frame_protocol.md
