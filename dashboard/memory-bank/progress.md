...
| 2.17.0  | **Frontend core/shared refactor (Namorix Core submodule).** Add submodule `vendor/namorix-core`. Bundle core tokens/base styles via SCSS sources; migrate toward `--nmx-*` tokens; add Vite alias `@namorix/core` → `vendor/namorix-core/src` for source consumption (no `dist/` required during dev). Store migrated to `createPluginStore` (`@namorix/core/store`) and Lit store controller unified in core (`LitStoreController`, `shallowEqual`). i18n runtime extracted to `@namorix/core/i18n` (translator + store-bound translator + locale storage); locale storage split into `frontend/src/core/i18n/locale-storage.ts` to avoid circular imports. Removed legacy Sass token file `frontend/src/styles/_variables.scss`; feature styles moved to CSS variables. |
