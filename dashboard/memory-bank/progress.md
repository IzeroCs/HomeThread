...
| 2.17.0  | **Core/shared refactor (unreleased).** Thêm submodule `vendor/namorix-core`. Frontend bundle core tokens/base styles qua Vite alias `@namorix/core` → `vendor/namorix-core/src` (import SCSS source khi `dist/` chưa build). Store chuyển sang `createPluginStore` từ `@namorix/core/store`; `LitStoreController` + `shallowEqual` dùng từ core (không còn bản copy). i18n runtime chuyển sang `@namorix/core/i18n` (`createStoreBoundTranslator`, locale storage), tách `core/i18n/locale-storage.ts` để tránh circular import store↔i18n. Dọn style: loại bỏ Sass global `_variables.scss`, chuyển sang CSS var/token-driven. |
