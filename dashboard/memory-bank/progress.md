...
| 2.19.1  | **Memory bank — tham chiếu Namorix Desktop.** Đồng bộ `dashboard/memory-bank/` với repo sibling **`namorix`** (Desktop shell): trỏ `namorix/documents/namorix-desktop-architecture.md` cho plugin-in-shell, manifest, CORS, `window.nmxCore`, milestone **M3**; kèm `namorix/memory-bank/` và `namorix-core/memory-bank/`. **Không đổi code** Thread — chỉ tài liệu. |
