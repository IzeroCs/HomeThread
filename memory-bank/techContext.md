...
- **Desktop registration debug:** Backend Thread expose `GET /api/desktop-registration-status` và field `addonRegister` trong `GET /health` để quan sát trạng thái gửi `register-request` (attempts, lastError, lastHttpStatus, registeredAt, stopped).
