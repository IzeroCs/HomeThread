...
| `@namorix/core-backend` | `file:` → `namorix/core/backend` | Logger (pino) + **`resolveNamorixRepoDataLayout(__dirname)`** cho SQLite/migrations (`<repo>/data/...`, override **`NAMORIX_DATA_DIR`**), plugin-generic helpers (`getOrCreateEnvStyleSecret`, `createDesktopPluginRegisterLoop`, shared registration/API types) và Full SDK `createPluginBackendServer` để gom plumbing Express/plugin runtime. Khai báo ở **`backend/package.json`** (`file:../../namorix/core/backend`) và root **`package.json`** (`file:../namorix/core/backend`) để workspaces hoist — **sau đổi dependency chạy `npm install` tại root repo Thread**. |

Transport: TCP (net.Socket) to BR; CoAP (UDP 5683, udp6 listen [::]) from Thread-Node. Dependencies: `coap`. **Env layer:** `backend/src/env.ts` là entry parse/load `.env` cho constants runtime (`PORT`, `DESKTOP_ORIGIN`, `DESKTOP_BACKEND_URL`, `PLUGIN_DEV_FRONTEND_URL`, `PLUGIN_PUBLIC_BASE_URL`, `BACKEND_IPV6`, `SRP_HOSTNAME`, `SRP_PORT`...). **Plugin backend plumbing** chuyển sang Full SDK core-backend: `backend/src/index.ts` dùng `createPluginBackendServer` để setup Express/CORS/static + `manifest`/`health`/`desktop-registration-status` + registration loop; secret vẫn do `backend/src/plugin-secret.ts` (wrapper `getOrCreateEnvStyleSecret`) cung cấp. Thread giữ domain runtime riêng (BrManager/WebSocket/CoAP/SRP). CBOR decode/encode noi bo (`backend/src/cbor`). Thread-Node la **CoAP client**: GET /device/ping**?mac=** (16-char hex, heartbeat → last_seen_at); POST /device/register/info (keys 0–6, key 0 = mac), /device/update/topology (payload **role-based**: child 0–5 parent_*; router/leader 0,1,2,6 neighbors array), /device/register/entity, /device/update/entity, /device/update/state (key 0 = mac, **key 1** = array). ENTITY_KEYS 0–6 (disabled key 6); STATE_KEYS 0–6 (không available). Spec payload: documents/coap/device_payload_spec.md; SRP: documents/coap/backend_discovery_srp.md; BR/routing: documents/architecture/real_br_integration.md. Backend luu **8 bảng** (device_info, device_topology, device_topology_neighbor, device_topology_history, device_entity, device_entity_state + history, **device_health_br** — 1 row per device, upsert via CMD_BR_HEALTH); upsertTopology parse theo role, replace neighbors; updateDeviceLastSeen, getDeviceStatus(30s/5m); **upsertBrHealth** (device-health.repository). Frame log: CMD STATE và ACK ẩn (br.command.ts).
...
- **Desktop registration debug:** Backend Thread expose `GET /api/desktop-registration-status` và field `pluginRegister` trong `GET /health` để quan sát trạng thái gửi `register-request` (attempts, lastError, lastHttpStatus, registeredAt, stopped).
