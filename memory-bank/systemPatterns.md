...
- **Desktop admin pending contract:** `shell:registrations:list` ack payload cho admin pending không còn `sourceIp`, chỉ còn `id/pluginId/baseUrl/manifest/createdAt` (host-side xử lý); Thread plugin không cần quan tâm trường này.
