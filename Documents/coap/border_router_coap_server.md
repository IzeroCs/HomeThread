...
## Flow register/info → register/entity → restore

1. **POST /device/register/info**: Backend upsert device_info (ON mac_address), generate slug nếu NULL, soft-delete entity + state cũ của device. Nếu payload có key 8 network thì ghi device_topology + device_topology_history.
2. **POST /device/register/entity**: Backend resolve device; upsert device_entity; restore state; query entity + state; build restore list. Response có thể kèm body **CBOR** (Content-Format application/cbor): map key **10** = array; mỗi phần tử là map với key 0=entity_id, 1=restore_mode, 2=state, 3=brightness, 4=mode, 5=rgb_json, 6=color_temp, 7=value_real, 8=has_saved_state. Node decode CBOR và áp dụng state/default cho từng entity.
