...
## Schema 8 bảng (device + entity + topology + BR health)
...
| device_health_br            | **Snapshot** health BR: **1 row per device** (UNIQUE(device_id)), upsert mỗi lần poll/notify. Cột: device_id, free_heap, minimum_free_heap, uptime, stack_hwm (text, TLV serialized), mle_detach_count, recorded_at. Điền bởi backend qua **frame CMD_BR_HEALTH** (TCP), không phải CoAP; backend poll ~60s và/hoặc khi NOTIFY bit health. |
