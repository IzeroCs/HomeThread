...
| ------ | ----------------------- | -------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| GET    | /device/ping            | query **?mac=**** (khuyến nghị)**                  | Backend trả 2.05 Content, body 4 byte timestamp uint32 LE. **Nên** gửi kèm `?mac=<eui64_hex>` (16 ký tự hex) để backend cập nhật heartbeat (last_seen_at). Node so sánh timestamp; đổi thì gửi lại register. |
...
| ----- | --------------- | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **0** | **mac_address** | bstr(8) | **Bắt buộc.** IEEE EUI-64 của giao diện **IEEE 802.15.4 (Thread)** (8 bytes, big-endian); Node lấy từ `esp_read_mac(..., ESP_MAC_IEEE802154)`. **Không đổi khi factory reset.** Backend chuyển sang 16-char hex (lower-case) để lưu DB. |
...
| Key   | Tên             | Kiểu    | Ghi chú                                                                            |
| ----- | --------------- | ------- | ---------------------------------------------------------------------------------- |
| **0** | **mac_address** | bstr(8) | **Bắt buộc.** IEEE EUI-64 (802.15.4/Thread), 8 bytes. Không đổi khi factory reset. |
...
**POST /device/register/entity** (contract chính): Request body map **key 0** = mac_address (**bstr(8)**, EUI-64), **key 1** = array (định nghĩa entity). Mỗi phần tử trong array 1 là map (định nghĩa entity) — **ENTITY_KEYS** (index 0–6):
...
Request body: **key 0** (mac_address **bstr(8)**) + **key 1** (array). Mỗi phần tử trong array 1 là map (state một entity) — **STATE_KEYS** (index 0–6):
...
- **Request:** GET `/device/ping` (không body). **Khuyến nghị:** gửi kèm query **?mac=**** (16 ký tự hex, ví dụ **`a1b2c3d4e5f60708`) để backend cập nhật **last_seen_at** (heartbeat) cho thiết bị.
...
1. **GET /device/ping?mac=**** — lấy timestamp (và cập nhật heartbeat); nếu timestamp khác lần trước → bắt đầu register.**
2. **POST /device/register/info** (keys 0–6, key 0 = mac_address bstr(8)) — gửi device info only. **Retry** đến khi backend trả 2.01/2.04 (không gửi register/entity cho đến khi register/info thành công).
3. **Chỉ khi register/info success** → **POST /device/register/entity** (key 0 = mac_address bstr(8), key 1 = array entity). Nhận restore (key 10) nếu backend trả.
4. **POST /device/update/topology** (role-based: child gửi 0,1,2,3,4,5; router/leader gửi 0,1,2,6) và **POST /device/update/state** (key 0 = mac_address bstr(8), key 1 = array state) — node gửi định kỳ sau khi đã register. **update/info** và **update/entity** chỉ dùng từ backend/UI, node không gửi.
