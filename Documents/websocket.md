...
- **handler/** — Handlers tách theo domain, mỗi class nhận dependencies qua constructor; method handler có `@WsOn(EVENTS.xxx)`.
...
1. Trong constructor WebSocketServer: tạo instance từng handler (truyền io, brConnectionConfigService, appSettingsService, communicate theo nhu cầu).
...
1. Thêm constant vào `shared/src/events.ts` (nếu chưa có).
2. Thêm method vào handler class phù hợp (config / br / device / thread / commissioner / srp), đánh dấu `@WsOn(EVENTS.TEN_EVENT)`.
