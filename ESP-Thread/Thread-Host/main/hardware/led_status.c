 ...
 * WS2812/WS2812B addressable RGB LED (1 chân data) qua RMT.
...
#define RMT_RESOLUTION_HZ  10000000  /* 10MHz cho ESP32-S3 */
#define BYTES_PER_PIXEL    3         /* GRB format cho WS2812 */
#define BRIGHTNESS         10        /* 0..255, tăng lên 30 cho ESP32-S3 để dễ nhìn hơn */
...
#define BLINK_MS    1200   /* nửa chu kỳ nhấp nháy (ms): ~1.2s sáng, 1.2s tắt */
#define TASK_MS     300    /* Update LED mỗi 100ms */
...
esp_err_t led_status_start(const led_status_config_t *config)
{
    ...
    rmt_tx_channel_config_t tx_cfg = {
        ...
        .trans_queue_depth = 2,
        .flags = {
            .invert_out = false,  // Không invert output
            .with_dma = false,    // Không dùng DMA cho ESP32-S3 (có thể bật nếu cần)
        },
    };
