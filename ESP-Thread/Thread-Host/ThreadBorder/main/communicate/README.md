...
**communicate_task** (Thread-Host): main gọi `communicate_task_start()` — init communicate + queue + RX callback (STATE→ACK + 1 byte role, lệnh khác→NACK) + state watchdog (backend gửi STATE interval; không nhận state trong 5 lần × 15s → restart). Backend pull định kỳ; BR không push. **communicate_queue:** process task **comm_proc** stack 4096 bytes, timeout gửi 500 ms (queue đầy → NACK 0x05 Busy), log khi item chờ xử lý &gt; 2 s; mỗi 30 s log stack high water mark (tag `communicate_queue`). **communicate_task:** state watchdog **state_wdg** stack 4069 bytes, mỗi ~30 s log stack high water mark (tag `communicate_task`). **communicate_command:** handler CMD_STATE, CMD_DATASET_ACTIVE, CMD_IP_ADDR (cache leader RLOC, lazy refresh), SET_*, ROUTER/CHILD/JOINER_TABLE, THREAD_START/STOP/VERSION. File: `communicate_task.c`, `communicate_queue.c`, `communicate_command.c`, header: `communicate_task.h`, `communicate_queue.h`, `communicate_command.h`.
