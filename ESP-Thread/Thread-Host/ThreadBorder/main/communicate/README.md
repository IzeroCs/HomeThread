...
- **`communicate_config.h`**
  - `COMMUNICATE_FRAME_PORT_IS_UART = 1`: frame trên UART, log trên CDC (hiện tại).
  - `= 0`: frame trên CDC, log trên UART (sau này khi có transport CDC).
...
- `communicate_init(rx_cb, rx_ctx)`: khởi tạo transport (UART), bắt đầu nhận frame; khi có frame hợp lệ gọi `rx_cb(frame_id, cmd, data, len, ctx)`.
