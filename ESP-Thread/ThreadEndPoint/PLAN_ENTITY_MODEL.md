...
**Không nằm trong phạm vi kế hoạch này**: giao thức Thread-specific (CoAP, Matter), chỉ tập trung UDP đơn giản trên port hiện tại (5684), format text hoặc JSON nhẹ.
...
## 5. Giao thức UDP (request / response)
...
## 6. Luồng khởi tạo và xử lý request
...
2. **UDP server** (trong task hiện tại):  
...
## 7. Cách thêm entity / type mới sau này
...
## 8. Thứ tự triển khai (khi bắt đầu code)
...
5. **main.c**: `entity_model_init()`, `entity_type_on_off_light_register()`, `led_udp_server_start()`.
...
- **Bảo mật**: UDP không bảo mật; nếu sau này cần xác thực/encryption thì đặt thêm lớp trên (vd: CoAP DTLS, Matter).  
...
- **Giao thức**: UDP text (describe / get / set), có thể chuyển sang JSON sau.  
- **Mở rộng**: thêm entity = gọi `entity_add`; thêm type = thêm file driver + register_type + add entity.  
- **Triển khai**: làm lần lượt entity_model → on_off_light → refactor UDP server → nối main và CMakeLists.

Khi đồng ý với kế hoạch này có thể bắt đầu bước 1 (entity_model.h).
