...
- **Backend coap/** (domain): `coap-device.server.ts` goi `registerCoapControllers(server, [DeviceCoapController])`. Controller dung decorator `@CoapGet("/device/ping")`, `@CoapPost("/device/register")`, `@CoapPost("/device/entities")`, `@CoapPost("/device/update")`. Metadata route luu tren constructor qua `coap.type.ts` (getCoapRoutes, appendCoapRoute); `coap.router.ts` doc routes, dispatch request → handler. File: `coap.type.ts`, `coap.decorator.ts`, `coap.router.ts`, `device-coap.controller.ts`, `device-coap.service.ts`, `device-register.payload.ts`.
...
- **Backend:** CoAP server UDP 5683 (`coap-device.server.ts`), socket **udp6** listen `[::]:5683`. Path **/device/** (ping, register, entities, update). **GET /device/ping**: tra **2.05 Content**, payload 4 byte timestamp uint32 LE (restart = timestamp moi → node gui lai register); **echo token** (RFC 7252). **POST /device/register**: payload CBOR **chi keys 0–8** (device + network, khong key 9); parse bang `backend/src/cbor`, upsert vao bang **device_info** (device-coap.service), tra **2.01 Created** / **2.04 Changed**, echo token. **POST /device/entities**: payload CBOR key **0** = device_id, key **9** = array entities; merge tung entity vao bang **device_entity** theo (device_id, entity_id); tra 2.01/2.04, echo token. **POST /device/update**: legacy, log CBOR, tra 2.01, echo token. Role trong payload la **so** (0=child, 1=router, 2=leader). **Khong emit** len frontend.
- **Registration model:** Thread-Node la **CoAP client** chu dong: POST register (device+network), POST entities (rieng); GET /device/ping dinh ky → neu timestamp khac thi gui lai register. Neu CoAP fail thi force SRP re-discovery va gui lai.
...
- **Docs:** `docs/coap/thread_node_coap.md` — luong Node → Backend, SRP discovery, payload register/entities; `docs/coap/border_router_coap_server.md` — spec Backend (endpoints, payload, device_info/device_entity).
...
- `backend/src/coap/device-coap.controller.ts` — GET /device/ping, POST /device/register (keys 0–8), POST /device/entities, POST /device/update; CBOR decode, echo token, tra 2.01/2.04/2.05
- `backend/src/coap/device-coap.service.ts` — upsertDevice(), mergeEntity(); SQLite bang device_info, device_entity
