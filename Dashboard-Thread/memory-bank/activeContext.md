...
- **Backend:** CoAP server UDP 5683 (`coap-device.server.ts`), socket **udp6** listen `[::]:5683`. Path **/device/** (ping, register/info, register/entity, update/info, update/entity, update/topology, update/state). **GET /device/ping**: tra **2.05 Content**, payload 4 byte timestamp uint32 LE; **POST /device/register/info**: CBOR keys 0–8, **mac_address (key 7)** bat buoc; upsert **device_info**, generate slug, soft-delete entity/state cu; neu co key 8 network thi ghi **device_topology** + history; tra 2.01/2.04. **POST /device/register/entity**: mac_address + key 9 array entities; merge **device_entity**, query restore; co the tra body **CBOR** (key 10 = array restore) de node ap dung state khi boot. **POST /device/update/info|entity|topology|state**: update device_info, entity definition, topology, entity state; tra 2.04. Moi response qua **sendCoapResponse** (echo token + status + optional body/Content-Format); status dung **CoapStatus** (coap.type.ts). **Khong emit** len frontend.
- **Schema:** device_info (mac_address TEXT UNIQUE, device_slug), device_topology + device_topology_history, device_entity (restore_mode, deleted_at), device_entity_state + device_entity_state_history (migration 009). Soft-delete khi register/info; restore flow khi register/entity (restore_mode 0–4).
...
- **Docs:** `docs/coap/thread_node_coap.md` — luong Node → Backend, SRP discovery, 6 URI, boot/restore flow; `docs/coap/border_router_coap_server.md` — spec Backend (endpoints, 6 bang, CoapStatus/sendCoapResponse/parseCborOrRespond).
...
- `backend/src/coap/device-coap.service.ts` — upsertDeviceInfo, updateDeviceInfo, upsertTopology, mergeEntity, updateEntityDefinition, upsertEntityState; SQLite 6 bang (device_info, device_topology, device_entity, device_entity_state + history)
