...
> Tài liệu chung — dùng cho cả **firmware** và **dashboard** (backend/frontend).

Mô tả format dữ liệu trả về từ Border Router (BR) cho các lệnh **Pull** để lấy Router Table, Child Table, và Commissioner Joiner Table.
...
function parseChildTable(data: Uint8Array): ChildEntry[] {
    ...
    for (let i = 0; i < count; i++) {
        entries.push({
            ...
            extAddress: Array.from(data.slice(offset + 3, offset + 11)),
            linkQualityIn: data[offset + 11],
            averageRssi: data[offset + 12] - 128, // unsigned → signed
...
#### Type (1 byte)
- `0x00` = `OT_JOINER_INFO_TYPE_ANY` (không có SharedId, padding 8 bytes 0x00)
...
- **Type = EUI64 (0x01):** 8 bytes EUI-64 address
- **Type = DISCERNER (0x02):** 1 byte length (bits, 1-64) + N bytes value (big-endian, N = ceil(length/8))
- **Type = ANY (0x00):** 8 bytes padding (0x00)

#### PSKD_length (1 byte) + PSKD (variable)
- Độ dài PSKD string (0-32); PSKD không null-terminated

#### ExpirationTime (4 bytes)
- uint32 big-endian, milliseconds; 0 = không expire
...
function parseJoinerTable(data: Uint8Array): JoinerEntry[] {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (type === 0x01) {
            entry.sharedId = { eui64: new Uint8Array(data.slice(offset, offset + 8)) };
            offset += 8;
        } else if (type === 0x02) {
            ...
            entry.sharedId = { discerner: { length: discernerLength, value: discernerValue } };
...
interface JoinerEntry {
    ...
    sharedId?: {
        ...
        discerner?: { length: number; value: bigint; };
    };
    pskd: string;            // UTF-8
...
2. **Buffer overflow:** Kiểm tra `LEN` trong frame header trước khi parse.
3. **Joiner Table:** Chỉ có dữ liệu khi BR đang ở chế độ Commissioner và có joiner entries. Nếu không: `count = 0`.
4. **Empty table:** `count = 0`, DATA chỉ có 1 byte.
5. **Error handling:** NACK `0x02` = Not ready; `0x03` = Timeout.
6. **Router Table:** Chỉ entries với `mAllocated == true`.
7. **Child Table:** Chỉ entries của children đang connected.

---

## Tài liệu liên quan

- **[usb_cdc_frame_structure.md](usb_cdc_frame_structure.md)** — Cấu trúc frame, CMD table, CRC8, Error codes.
